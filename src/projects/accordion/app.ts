...
const accordionContainer = document.querySelector('.accordion-container')! as HTMLDivElement;
const accordionContent = accordionData
  .map((item: TAccordionData) => {
...
const accordionItem = document.querySelectorAll('.accordion-item')! as NodeListOf<HTMLDivElement>;
accordionItem.forEach((item: HTMLDivElement, index: number) => {
  const accordionHeader = item.querySelector('.accordion-header')! as HTMLDivElement;
  accordionHeader.addEventListener('click', () => {
    item.classList.toggle('open');
    const accordionDescription = item.querySelector('.accordion-description')! as HTMLParagraphElement;
...
function removeOpenClass(currentIndex: number) {
  accordionItem.forEach((item: HTMLDivElement, index: number) => {
